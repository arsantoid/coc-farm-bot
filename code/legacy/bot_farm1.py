...
# ╚══════════════════════════════════════════════════════════════╝
INDEX_AKUN_MULAI = 6
STOP_AFTER_INDEX = -1          # -1 = sampai akhir daftar. Misal 6 = stop setelah index 6 (MossyMinion 4)
LOOP_CONTINUOUSLY = False       # True = ulangi terus dari awal, False = berhenti setelah daftar/stop index selesai
...
# ── CONNECTION LOST ──
LOADING_TIMEOUT = 10          # Max detik tunggu loading awan
MATCH_FIND_TIMEOUT = 5       # Max detik tunggu attack_setup muncul
    ...
    for spell_file in SPELL_TEMPLATES:
        ...
        if max_val >= 0.80:
            ...
            if SPELL_DEPLOY_TARGETS:
                ...
                print(f"   🔮 {spell_file} ({max_val:.2f}) → ({tx},{ty})")
                time.sleep(0.5)
...
def clear_popups_and_recover():
    """Bersihkan SEMUA popup secara agresif."""
    print("🧹 Membersihkan pop-up...")
    
    # 1. Tutup event popups dulu
    clear_event_popups()
    
    # 2. Klik tombol Okay berkali-kali (handle stacked popups)
    for i in range(5):
        if find_and_click("okay_surrender.png", threshold=0.75):
            print(f"    Okay clicked ({i+1}/5)")
            time.sleep(0.8)
        else:
            break
    
    # 3. Klik close X berkali-kali
    for i in range(5):
        if find_and_click("close_x.png", threshold=0.75):
            print(f"   ❌ Close X clicked ({i+1}/5)")
            time.sleep(0.8)
        else:
            break
    
    # 4. Klik pojok kanan atas berkali-kali (catch-all untuk popup apapun)
    for i in range(3):
        device.shell("input tap 850 50")
        time.sleep(0.5)
    
    # 5. Klik tengah layar juga (untuk popup yang tombolnya di tengah)
    for i in range(2):
        device.shell("input tap 480 300")
        time.sleep(0.5)
...
# ╔══════════════════════════════════════════════════════════════╗
# ║              MASTER LOOP (UPDATED v9)                       ║
# ╚══════════════════════════════════════════════════════════════╝
def main_farming_loop():
    print()
    print("🚀 " + "=" * 53)
    print("   BOT FARMING v9 — LOOP & STOP CONTROL")
    print("=" * 55)
    ...
    is_first_run = True  # Penanda agar tidak nge-switch akun di detik pertama

    # ╔══════════════════════════════════════════════════════════════╗
    # ║ OUTER LOOP: Mengatur pengulangan terus-menerus (Continuous)  ║
    # ╚══════════════════════════════════════════════════════════════╝
    while True:
        
        # ╔══════════════════════════════════════════════════════════════╗
        # ║ INNER LOOP: Iterasi farming per akun                         ║
        # ╚══════════════════════════════════════════════════════════════╝
        while current_index < total_akun:
            ...
            # LOGIKA SWITCH AKUN
            if not is_first_run:
                # Jika bukan run pertama, kita harus switch akun dulu
                ...
                # 🔥 FIX: Clear popups SEBELUM zoom out
                print("🧹 Membersihkan pop-up sebelum zoom...")
                clear_popups_and_recover()
                time.sleep(1)
            else:
                # Untuk akun PERTAMA di awal skrip, langsung bersihkan popup
                print("🧹 Membersihkan pop-up awal sebelum zoom...")
                clear_popups_and_recover()
                time.sleep(1)
                is_first_run = False  # Ubah jadi False agar akun berikutnya di-switch
            ...
            # ╔══════════════════════════════════════════════════════════════╗
            # ║ BATTLE LOOP: Farming di akun ini sampai penuh/timeout        ║
            # ╚══════════════════════════════════════════════════════════════╝
            ...
            # Pindah ke index akun berikutnya
            current_index += 1

        # ══════════════════════════════════════════════════════════════════
        # AKHIR DARI INNER LOOP (Semua akun sudah diproses atau kena stop)
        # ══════════════════════════════════════════════════════════════════
        
        # Jika looping dimatikan, berhenti total
        if not LOOP_CONTINUOUSLY:
            print(" Looping dimatikan. Bot berhenti.")
            break
            
        # Jika looping aktif, reset index ke 0 dan ulangi
        print(" Mengulang dari akun awal...")
        current_index = 0
        is_first_run = False # Tetap False karena kita sudah di akun terakhir, perlu switch ke akun 0
        time.sleep(5) 
    ...
    print("=" * 55)
    logger.info(f"Bot selesai: {total_battle_semua} battle, {menit_total}m {detik_total}s")
