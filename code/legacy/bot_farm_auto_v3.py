...
BUILDING_TEMPLATES = [
    ...
    ("building_barracks.png",            "Barracks",            False, 3),
    ("building_dark_spell_factory.png",  "Dark Spell Factory",  False, 3),
    ...
    ("building_elixir_storage.png",      "Elixir Storage",      False, 5),
    ("building_gold_storage.png",        "Gold Storage",        False, 5),
    ("building_dark_elixir_storage.png", "Dark Elixir Storage", False, 5),
...
def find_confirm_button_on_screen(screen, context="all", threshold=CONFIRM_THRESHOLD):
    """Cari confirm button di screenshot yang sudah diambil. Return (x, y, conf) atau None."""
    if context == "hero":
    ...
    for tpl in templates:
        result = find_on_screen(tpl, screen, threshold)
        if result:
...
def find_on_screen(template_name, screen, threshold=0.85):
    """Find template di screenshot yang sudah diambil. Return (x, y, conf) atau None."""
    img_path = os.path.join(GAMBAR_DIR, template_name)
    if not os.path.exists(img_path):
        return None
    template = cv2.imread(img_path, cv2.IMREAD_COLOR)
...
def scan_buildings_in_menu(screen):
    ...
    for tpl_file, label, is_hero, priority in BUILDING_TEMPLATES:
        pos = find_on_screen(tpl_file, screen, threshold=DETECT_THRESHOLD)
        if pos is None:
            continue
        cx, cy, conf = pos
        if is_in_menu(cx, cy):
...
def auto_upgrade_suggested_fallback(context="builder"):
    ...
    for attempt in range(MAX_SCROLLS + 1):
        ...
        header = find_on_screen(TEMPLATE_HEADER, screen, threshold=HEADER_THRESHOLD)
        if header is None and attempt > 0:
            break

        upgrade = find_on_screen(TEMPLATE_UPGRADE, screen, threshold=UPGRADE_THRESHOLD)
        if upgrade is None:
            break

        ux, uy, uconf = upgrade

        if not is_in_menu(ux, uy):
            break

        click_x = ux
        click_y = uy + 28
        tap(click_x, click_y)
        print(f"   🎯 Suggested: tap ({click_x}, {click_y}) conf={uconf:.2f}")
        time.sleep(DELAY_AFTER_TAP)

        if find_template(TEMPLATE_BUSY, BUSY_THRESHOLD):
            print(f"   🔴 Builders busy (suggested)")
            tap(CLOSE_X, CLOSE_Y)
            time.sleep(DELAY_AFTER_CLOSE)
            break

        confirm = find_confirm_button_on_screen(screen, context="all")
        if not confirm:
        ...
        if find_template(TEMPLATE_HEY_CHIEF, HEY_CHIEF_THRESHOLD):
...
def auto_upgrade_building():
    ...
    for scroll_idx in range(MAX_SCROLLS + 1):
        ...
        has_upgrade = find_on_screen(TEMPLATE_UPGRADE, check_screen, threshold=UPGRADE_THRESHOLD)
...
def main_farming_loop():
    ...
    while True:
        ...
        while current_index < total_akun:
            ...
            # ── SIMPAN PROGRESS SEBELUM MULAI AKUN ──
            ...
            # ═══════════════════════════════════════════════════════
            # SIKLUS: FARM → UPGRADE (berulang)
            # ═══════════════════════════════════════════════════════
            for cycle in range(MAX_FARM_UPGRADE_CYCLES):
        ...
        if not LOOP_CONTINUOUSLY:
            ...
            clear_progress()  # Semua selesai, hapus progress
            break

        print(" Mengulang dari akun awal...")
        clear_progress()  # Semua selesai, hapus sebelum mulai ulang
        current_index = 0
        is_first_run = False
        time.sleep(5)
